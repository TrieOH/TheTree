...
var (
	...
	ErrSDKInvalidObjectFormat       = fail.Form(SDKInvalidObjectFormatID, "invalid object format '%s': must be plain name (e.g., 'document', 'user_profile') or global wildcard '*', without separators", false, map[string]any{"code": 400})
	ErrSDKInvalidActionFormat       = fail.Form(SDKInvalidActionFormatID, "invalid action format '%s': must be plain name (e.g., 'read', 'write') or global wildcard '*', without separators", false, map[string]any{"code": 400})
