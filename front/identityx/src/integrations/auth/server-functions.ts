...
export const isSetupDoneServerFn = createServerFn({ method: "GET" }).handler(
  () => bff.isSetupDone(),
);
...
export const introspectServerFn = createServerFn({ method: "POST" })
  .validator(z.object({ apiKey: z.string().optional() }).optional())
  .handler(({ data }) => bff.introspect(data?.apiKey));
