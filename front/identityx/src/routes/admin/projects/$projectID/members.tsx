...
import {
  addMemberToProjectFn,
  allProjectMembersQueryOptions,
  removeMemberFromProjectFn,
} from "@/features/project/api";
import {
  type MemberAddToProjectI,
  memberAddToProjectSchema,
  type ProjectMemberI,
...
function RouteComponent() {
  ...
  const { data: members = [] } = useQuery(
    allProjectMembersQueryOptions(projectID, organizationID),
  );

  const [filter, setFilter] = useState("");
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [memberToRemove, setMemberToRemove] = useState<ProjectMemberI | null>(
    null,
  );
  ...
  const { mutate: removeMemberFromProject, isPending: isRemoving } =
    useMutation({
