import { allCapabilitiesQueryOptions, createCapabilityFn } from '@/features/capabilities/api'
import { capabilityCreateSchema, type CapabilityCreateI, type CapabilityI } from '@/features/capabilities/model'
...
function RouteComponent() {
  ...
  const { mutate: createCapability, isPending: isCreating } = useMutation({
    mutationFn: (data: CapabilityCreateI) => createCapabilityFn(projectID, data),
    onSuccess: (response) => {
      if (response.success) {
    ...
    <div>
      <PaginatedContainer<CapabilityI>
        ...
        renderItems={(slice) => slice.map((item) => (
          <CapabilityCard key={item.id} data={item} />
        ))}
      />
