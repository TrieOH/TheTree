import { createActorFn, allActorsQueryOptions } from "@/features/actor/api";
import { actorCreateSchema, type ActorCreateI, type ActorI } from "@/features/actor/model";
...
function RouteComponent() {
  ...
  const { data: actors = [] } = useQuery(allActorsQueryOptions(projectID, organizationID))
  ...
  const { mutate: createActor, isPending: isCreating } = useMutation({
    mutationFn: (data: ActorCreateI) => createActorFn(projectID, data, organizationID),
    onSuccess: (response) => {
      if (response.success) {
  ...
  return (
    <div>
      ...
      <FormModal<ActorCreateI>
        ...
        defaultValues={{ auth_method: 'password', type: 'human', email: undefined }}
