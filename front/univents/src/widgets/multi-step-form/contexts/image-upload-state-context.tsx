...
const ImageUploadStateContext = createContext<ImageUploadStateContextValue | null>(null);

export function ImageUploadStateProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<ImageUploadStateContextValue>(
    () => ({
      ...
      setItems: (key, items) => {
        setState((current) => (current[key] === items ? current : { ...current, [key]: items }));
      },
    }),
    [state],
  );

  return <ImageUploadStateContext.Provider value={value}>{children}</ImageUploadStateContext.Provider>;
}

export function useImageUploadState() {
  const context = useContext(ImageUploadStateContext);
  if (!context)
    throw new Error("useImageUploadState must be used within ImageUploadStateProvider");
  return context;
}
