...
export function ImageFieldRenderer<TFieldValues extends FieldValues>({
  ...
  const { items, addFiles, removeItem } = useImageUploadField({
    ...
    onValueChange: (urls) => {
      form.setValue(field.name, (urls[0] ?? null) as never, { shouldDirty: true });
    },
  ...
  return (
    <div className="space-y-1.5">
      ...
      >
        {hasItem ? (
          <ImageThumb item={item} onRemove={() => removeItem(item.id)} className="absolute inset-0 h-full w-full rounded-none border-0" />
        ) : (
          <button
            type="button"
          ...
          >
            ...
            {field.hint ? <p className="max-w-sm text-xs text-muted-foreground">{field.hint}</p> : null}
          </button>
        )}
