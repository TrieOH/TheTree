import type { Canvas, FabricImage, Rect as FabricRect, Textbox } from "fabric";
import { useCallback, useEffect, useRef, useState } from "react";
import { parseRichTextMarkup, plainIndexToMarkupIndex } from "./rich-text";
import type {
  CanvasElement,
  ImageCanvasElement,
  TextCanvasElement,
...
type CanvasObjectRole = "text" | "field" | "label" | "preview";

type CanvasObjectWithData<T> = T & {
  data?: {
    elementId: string;
    role: CanvasObjectRole;
  };
};
...
function setCanvasObjectData<T extends object>(
  object: T,
  data: { elementId: string; role: CanvasObjectRole },
) {
  (object as T & { data?: typeof data }).data = data;
}

interface CanvasElementObjects {
  field: CanvasObjectWithData<FabricRect> | null;
  label: CanvasObjectWithData<Textbox>;
  preview: CanvasObjectWithData<FabricImage> | null;
}
...
export function useCertificateCanvas({
  ...
  const resolveSignatureUrl = useCallback(
  ...
  const syncTextElementFromTextbox = useCallback(
    (elementId: string, tb: Textbox) => {
      ...
      while (
        prefix < maxPrefix &&
        currentPlainText[prefix] === nextPlainText[prefix]
      ) {
        prefix += 1;
      }
      ...
      if (nextContent !== textEl.content) {
        onElementsChange(
          elementsRef.current.map((el) =>
            el.id === elementId
              ? ({ ...el, content: nextContent } as CanvasElement)
              : el,
          ),
        );
      }
  ...
  const updateLabelPosition = useCallback(
    (id: string, el: CanvasElement) => {
      ...
      if (content) {
        const s = Math.min(
          (fw * 0.88) / (content.width || 1),
          (fh * 0.88) / (content.height || 1),
        );
  ...
  const syncBackgroundImage = useCallback(
    async (targetCanvas?: Canvas | null) => {
      ...
      try {
        ...
        if (token !== backgroundLoadTokenRef.current || !canvasRef.current)
          return;
  ...
  const createElementOnCanvas = useCallback(
    (
      ...
      if (element.type === "text") {
        ...
        setCanvasObjectData(tb, { elementId: element.id, role: "text" });
        tb.initDimensions();
        ...
        objectsRef.current.set(element.id, {
          field: null,
          label: tb,
          preview: null,
        });
      ...
      const field = new Rect({
        ...
        width: w,
        height: h,
        rx: isSignature ? 22 : 10,
        ry: isSignature ? 22 : 10,
      ...
      setCanvasObjectData(field, { elementId: element.id, role: "field" });
      ...
      setCanvasObjectData(label, { elementId: element.id, role: "label" });
  ...
  const loadImageForElement = useCallback(
    async (elementId: string, src: string) => {
      const objs = objectsRef.current.get(elementId);
      const canvas = canvasRef.current;
      if (!objs || !canvas) return;
      if (objs.preview) {
        canvas.remove(objs.preview);
        objs.preview = null;
      }
      try {
        beginAssetLoad();
        ...
        setCanvasObjectData(img, { elementId, role: "preview" });
        canvas.add(img);
  ...
  const syncFull = useCallback(() => {
    ...
    for (const element of elements) {
      ...
      const w =
        element.type === "text"
          ? clamp(cw * (element.widthPct / 100), 60, cw * 0.85)
          : clamp(cw * (element.widthPct / 100), 60, cw * 1.5);
      const h =
        element.type === "text"
          ? 40
          : clamp(ch * (element.heightPct / 100), 30, ch * 0.8);
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      window.__fabricCanvas = instance;
      fabricRef.current = {
        Rect: fabricModule.Rect,
        Textbox: fabricModule.Textbox,
      };
      ...
      instance.setViewportTransform(
        getViewportTransform(instance.getWidth(), instance.getHeight()),
      );
      instance.renderAll();
      ...
      const observer = new ResizeObserver(() => {
        ...
        canvasRef.current.setViewportTransform(
          getViewportTransform(width, height),
        );
  ...
  useEffect(() => {
    ...
    async function updatePreviews() {
      ...
      try {
        ...
        for (const [, objs] of objectsRef.current) {
          if (objs.preview) {
            cur.remove(objs.preview);
            objs.preview = null;
          }
        }
        for (const [id, objs] of objectsRef.current) {
          ...
          try {
            ...
            setCanvasObjectData(img, { elementId: id, role: "preview" });
            objs.preview = img;
            cur.add(img);
