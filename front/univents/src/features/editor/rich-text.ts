...
export function plainIndexToMarkupIndex(
  parsed: ParsedRichText,
  plainIndex: number,
): number {
  if (plainIndex <= 0) return 0;
  if (plainIndex >= parsed.plainToMarkupIndex.length)
    return parsed.sourceLength;
...
export function toggleItalicMarkup(
  value: string,
  selectionStart: number,
  selectionEnd: number,
): MarkupChangeResult {
...
export function toggleUnderlineMarkup(
  value: string,
  selectionStart: number,
  selectionEnd: number,
): MarkupChangeResult {
