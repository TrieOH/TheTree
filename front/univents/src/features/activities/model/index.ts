...
export const activityCreateSchema = z.object({
  title: requiredText("Título").min(3, {
  ...
  token_cost: z.coerce
    .number({ error: "Custo em tokens é obrigatório" })
    .int({ message: "Custo em tokens precisa ser um número inteiro" })
    .nonnegative({ message: "Custo em tokens não pode ser negativo" }),
  has_capacity: z.boolean().default(false),
  capacity: z.coerce
    .number({ error: "Capacidade é obrigatória" })
    .int({ message: "Capacidade precisa ser um número inteiro" })
