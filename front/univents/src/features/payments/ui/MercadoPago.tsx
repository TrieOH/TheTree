...
import { AlertCircle, Loader2, Lock, QrCode } from "lucide-react";
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-5 gap-3">
        <div className="space-y-1.5 col-span-2">
          <Label
            htmlFor="pix-identification-type"
            className="text-xs font-medium text-muted-foreground uppercase tracking-wide"
          >
            Tipo
          </Label>
        ...
        <div className="space-y-1.5 col-span-3">
          <Label
            htmlFor="pix-identification-number"
            className="text-xs font-medium text-muted-foreground uppercase tracking-wide"
          >
      ...
      <Button
        onClick={() => {
          onSubmit(email, identificationType, identificationNumber);
        }}
        disabled={!canSubmit}
        ...
        const cardFormInstance = mp.cardForm({
          ...
          callbacks: {
            ...
            onSubmit: (event) => {
              ...
              if (
                !validationRef.current.isEmailValid ||
                !validationRef.current.isDocValid
              )
                return;
  ...
  const handleCardSubmit = (data: PaymentPayload) => {
    ...
    try {
      handleSubmit({
        ...
        identification_number: data.payer.identification.number.replace(
          /\D/g,
          "",
        ),
      });
