...
import { variantCreateSchema, type VariantCreateInputI, type VariantCreateOutputI, type VariantI } from '../model'
...
export interface ManageVariantModalProps {
  ...
  productId: string
  variant?: VariantI
  onCreate?: (values: VariantCreateOutputI) => Promise<VariantI | null | boolean>
  onUpdate?: (id: string, values: VariantCreateOutputI) => Promise<VariantI | null | boolean>
}
...
export function ManageVariantModal({
  ...
  const editValues = useMemo(() => (variant ? toFormValues(variant) : undefined), [variant])
