  ...
  const visibleForms = useMemo(() => {
    return forms.filter((form) => {
      if (statusFilter === "archived")
        return form.status === FormStatusArchived;
      ...
      <PaginatedContainer<FormI>
        ...
        sortFields={[
          ...
          { key: "updated_at", label: "Updated At" },
          {
            ...
            label: "Scope",
            comparator: (a, b) =>
              Number(Boolean(b.namespace_id)) - Number(Boolean(a.namespace_id)),
          },
        ]}
        filterValue={filter}
            ...
            <div className="flex h-9 items-center gap-1 rounded-md border border-border bg-background p-1">
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setStatusFilter(option.key as "all" | "active" | "archived")
                  }
                  className={[
        ...
        renderItems={(slice) =>
          slice.map((item) => {
            return <FormCard key={item.id} data={item} />;
          })
        }
      />
