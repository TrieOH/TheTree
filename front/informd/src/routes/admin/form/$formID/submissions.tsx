...
function SubmissionsComponent() {
  ...
  const { data: fullForm = null, isLoading: isFullFormLoading } = useQuery(
    allFormsResponsesQueryOptions(formID, namespaceID),
  );

  const submissions = useMemo(
    () => (fullForm ? deriveSubmissions(fullForm) : []),
    [fullForm],
  );
  ...
  const header = useMemo(() => {
    ...
    return (
      <FormAdminHeader
        ...
        description={
          responseCount === 0
            ? `No submissions yet for ${form.title}`
            : `${responseCount} submission${responseCount !== 1 ? "s" : ""} for ${form.title}`
        }
        form={form}
      ...
      <div className="flex flex-col items-center justify-center min-h-100 gap-4 p-8 text-center">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground max-w-62.5">
            We couldn't find the form you're looking for or it might have been
            deleted.
          </p>
        </div>
      </div>
