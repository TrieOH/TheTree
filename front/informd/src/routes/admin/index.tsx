...
function RouteComponent() {
  ...
  const namespacesWithOwnership = uniqueNamespaces.map((namespace) => ({
    ...namespace,
    ownership: namespace.owner_id === user_id ? "owner" : "member",
  }));
  ...
  return (
    <div className="flex flex-wrap p-4">
      <PaginatedContainer<NamespaceI & { ownership: "owner" | "member" }>
        items={
          filteredNamespaces as (NamespaceI & {
        ...
        renderItems={(slice) =>
          slice.map((item) => <NamespaceCard key={item.id} data={item} />)
        }
      />
