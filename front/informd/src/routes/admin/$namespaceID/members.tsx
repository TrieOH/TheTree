...
import type {
  ...
  NamespaceMemberI,
} from "#/features/namespaces/model";
...
function RouteComponent() {
  ...
  const { data: members = [] } = useQuery(
    allNamespacesMembersQueryOptions(namespaceID),
  );

  const [filter, setFilter] = useState("");
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [memberToRemove, setMemberToRemove] = useState<NamespaceMemberI | null>(
    null,
  );
  ...
  const { mutate: addMemberToNamespace, isPending: isCreating } = useMutation({
    mutationFn: (data: MemberAddToNamespaceI) =>
      addMemberToNamespaceFn(namespaceID, data),
    onSuccess: (response, variable) => {
      if (response.success) {
        if (user_id) {
          queryClient.setQueryData(
            allNamespacesMembersQueryOptions(namespaceID).queryKey,
            (oldData: NamespaceMemberI[] = []) => [
              ...oldData,
              {
  ...
  const { mutate: removeMemberFromNamespace, isPending: isRemoving } =
    useMutation({
      mutationFn: (rm_user_id: string) =>
        removeMemberFromNamespaceFn(namespaceID, rm_user_id),
      onSuccess: (response, rm_user_id) => {
        if (response.success) {
          queryClient.setQueryData(
            allNamespacesMembersQueryOptions(namespaceID).queryKey,
            (oldData: NamespaceMemberI[] = []) =>
              oldData.filter((member) => member.user_id !== rm_user_id),
          );
