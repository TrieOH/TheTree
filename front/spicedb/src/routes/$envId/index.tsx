...
export const Route = createFileRoute("/$envId/")({
  loader: async ({ context, params }) => {
    return context.queryClient.fetchQuery(schemaQueryOptions(params.envId));
  },
...
function SchemaEditorPage() {
  ...
  const schemaText = query.data.schemaText;
  ...
  useEffect(() => {
    ...
    if (!isDirty) setValue(schemaText);
  }, [schemaText, isDirty, envId]);

  const mutation = useMutation({
    mutationFn: async (newSchema: string) => {
      const res = await writeSchema({ data: { schema: newSchema, envId } });
