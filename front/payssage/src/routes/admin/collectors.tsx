...
export const Route = createFileRoute("/admin/collectors")({ component: CollectorsPage })
  ...
  return (
    <div>
      <div className="border-b border-border/40 bg-background px-6 py-4">
        <div>
          <h1 className="text-lg font-semibold tracking-tight">My collectors</h1>
          <p className="text-sm text-muted-foreground">
            {data.length} collector{data.length === 1 ? "" : "s"} connected · {activeCount} active
          </p>
        </div>
      </div>
      <div className="space-y-6 p-6">
        <ProviderConnectSection flow="collector" />
        <ProviderCredentialList items={data} flow="collector" queryKey={collectorsQueryOptions().queryKey} />
      </div>
    </div>
  )
}
