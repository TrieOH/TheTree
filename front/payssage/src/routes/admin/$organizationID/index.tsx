...
function RouteComponent() {
  ...
  const { mutate: setWalletFee, isPending: isSettingFee } = useMutation({
    mutationFn: ({ walletId, data }: { walletId: string; data: WalletSetFeeBpsI }) =>
    ...
    onSuccess: (response, variables) => {
      if (response.success) {
        ...
        queryClient.setQueryData(
          allWalletsQueryOptions(organizationID).queryKey,
          (old: WalletI[] = []) =>
            old.map((wallet) => (wallet.id === walletId ? { ...wallet, fee_bps: data.fee_bps } : wallet)),
        )
  ...
  const { mutate: setWalletSandbox, isPending: isSettingSandbox } = useMutation({
    mutationFn: ({ walletId, data }: { walletId: string; data: WalletSetSandboxI }) =>
    ...
    onSuccess: (response, variables) => {
      if (response.success) {
        ...
        queryClient.setQueryData(
          allWalletsQueryOptions(organizationID).queryKey,
          (old: WalletI[] = []) =>
            old.map((wallet) => (wallet.id === walletId ? { ...wallet, sandbox: data.sandbox } : wallet)),
        )
        toast.success(response.message || 'Wallet sandbox state updated successfully')
      } else toast.error(response.message || 'Failed to update wallet sandbox state')
    },
    onError: (error: Error) => toast.error(error.message),
  })
