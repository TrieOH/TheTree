...
export const getProviderCallbackUrl = (provider: string) =>
  new URL(`/callback/${encodeURIComponent(provider)}`, window.location.origin).toString()
...
export const getProviderCallbackFn = (code: string, state: string, provider: string) =>
