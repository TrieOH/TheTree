...
const formatAmount = (intent: Intent) => new Intl.NumberFormat(undefined, {
  style: "currency",
...
const shortId = (value: string) => value.length > 22 ? `${value.slice(0, 12)}…${value.slice(-6)}` : value

const formatBRL = (amountCents: number) => new Intl.NumberFormat(undefined, {
  style: "currency",
  currency: "BRL",
    ...
    <article className="w-full rounded-lg bg-card p-4 ring-1 ring-foreground/10">
      <div className="flex min-w-0 items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs capitalize text-muted-foreground">{intent.provider.replaceAll("_", " ")}</p>
          <p className="mt-1 text-xl font-semibold tracking-tight">{formatAmount(intent)}</p>
        </div>
      ...
      <div className="mt-5 space-y-2 text-xs">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <span className="truncate text-right">{formatDate(intent.created_at)}</span>
        </div>
      </div>
    </article>
  )
...
export function TransactionsDashboard({ title = "Transactions", description = "Payment activity across your wallets.", walletId }: {
  ...
  const testVolume = succeeded
    .filter((intent) => intent.sandbox)
    .reduce((total, intent) => total + intent.amount_cents, 0)
  const successRate = intents.length ? Math.round((succeeded.length / intents.length) * 100) : 0
  ...
  const filtered = environmentIntents.filter((intent) => {
    ...
    const matchesSearch = !term || [intent.id, intent.wallet_id, intent.provider, intent.status].some((value) => value.toLowerCase().includes(term))
  ...
  return (
    <div className="min-w-0 space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        ...
        <div className="flex w-fit rounded-md bg-muted p-1" aria-label="Transaction environment">
          {(["all", "production", "test"] as const).map((environment) => (
            <Button
              ...
              className={cn(
                "h-8 px-3 text-xs",
                environmentFilter === environment && "bg-background text-foreground shadow-xs hover:bg-background",
              )}
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat) => (
          <div key={stat.label} className="rounded-lg bg-card p-4 ring-1 ring-foreground/10">
      ...
      <section className="rounded-lg bg-card p-4 ring-1 ring-foreground/10">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Current intent distribution</p>
          </div>
          <span className="text-xs text-muted-foreground">{environmentIntents.length} total</span>
        </div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {intentStatuses.map((status) => {
            const count = environmentIntents.filter((intent) => intent.status === status).length
      ...
      <section className="space-y-3">
        ...
        <PaginatedContainer<Intent>
          ...
          renderItems={(items) => items.map((intent) => (
