...
import {
  bindCollectorToWalletFn,
  unbindCollectorFromWalletFn,
  walletByIdQueryOptions,
} from "../api";
...
export function WalletCollector({ wallet }: { wallet: WalletI }) {
  ...
  const { data: collectors = [] } = useQuery(
    collectorsQueryOptions(wallet.organization_id),
  );
  const [collectorId, setCollectorId] = useState(wallet.collector_id ?? "");

  useEffect(
    () => setCollectorId(wallet.collector_id ?? ""),
    [wallet.collector_id],
  );

  const refreshWallet = () =>
    queryClient.invalidateQueries({
      queryKey: walletByIdQueryOptions(wallet.id).queryKey,
    });
  const { mutate: bind, isPending: isBinding } = useMutation({
    mutationFn: () =>
      bindCollectorToWalletFn(wallet.id, { collector_id: collectorId }),
    onSuccess: (response) => {
      if (!response.success)
        return toast.error(response.message || "Failed to bind collector");
      void refreshWallet();
      toast.success(
        wallet.collector_id ? "Collector changed" : "Collector bound",
      );
    },
  ...
  const { mutate: unbind, isPending: isUnbinding } = useMutation({
    ...
    onSuccess: (response) => {
      if (!response.success)
        return toast.error(response.message || "Failed to unbind collector");
      setCollectorId("");
      void refreshWallet();
  ...
  const selectedCollector = activeCollectors.find(
    ...
    <section className="space-y-3 rounded-sm border bg-card p-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Bind one collector to process payments for this wallet.
        </p>
      </div>
      <div className="space-y-2">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <DropdownMenu>
            <DropdownMenuTrigger
              render={
                <Button
                  variant="outline"
                  className="min-h-10! flex-1 justify-between rounded-sm px-3 py-2 font-normal"
                />
              }
            >
              {selectedCollector ? (
                <span className="flex min-w-0 items-center gap-2">
                  <span className="capitalize">
                    {selectedCollector.provider.replaceAll("_", " ")}
                  </span>
                  <span className="truncate font-mono text-xs text-muted-foreground">
                    {selectedCollector.provider_user_id}
                  </span>
                </span>
              ) : (
            ...
            <DropdownMenuContent className="">
              ...
              {activeCollectors.map((collector) => (
                <DropdownMenuItem
                  key={collector.id}
                  onClick={() => setCollectorId(collector.id)}
                >
                  <span className="flex min-w-0 flex-1 flex-col">
                    <span className="capitalize">
                      {collector.provider.replaceAll("_", " ")}
                    </span>
                    <span className="truncate font-mono text-xs text-muted-foreground">
                      {collector.provider_user_id}
                    </span>
                  </span>
          ...
          <Button
            disabled={
              !collectorId || collectorId === wallet.collector_id || isBinding
            }
            onClick={() => bind()}
          >
            <Link2 />{" "}
      ...
      {!activeCollectors.length && (
        <p className="text-xs text-muted-foreground">
          No active collectors are available for this wallet.
        </p>
      )}
    </section>
  );
