...
export function ProfileView(props: ProfileViewProps): JSX.Element {
  ...
  const queryError = createMemo(() => query().error as (Error & {
    code?: number;
  ...
  const isMissingOwnProfile = createMemo(
    () => query().isError && own() && errorCode() === 404,
  );
  const errorMessage = createMemo(() => {
    if (isMissingOwnProfile()) {
  ...
  return (
    <Show when={!query().isPending} fallback={<ProfileSkeleton ownProfile={own()} />}>
      <Show
        when={!query().isError}
        fallback={
...
function ProfileSkeleton(props: { ownProfile: boolean }) {
  return (
