  ...
  return (
    <>
      <Reveal delay={(props.index ?? 0) * 0.04} animate={props.animate} class="h-full">
        <article
          class={cn(
            "group relative flex h-full w-full min-w-0 flex-col rounded-lg border border-border/60 bg-card text-left transition-colors duration-150 hover:border-border",
          )}
        >
          {/* Certificate Preview Header (Compact style aligned with AdminBadgeCard) */}
          <div class="relative flex h-36 w-full items-center justify-center overflow-hidden rounded-t-lg bg-muted/30 p-3">
            <CertificatePreview
            ...
            {/* Template Actions (Subtle icon buttons) */}
