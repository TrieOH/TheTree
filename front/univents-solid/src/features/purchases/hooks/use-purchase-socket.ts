...
export function usePurchaseSocket(purchaseId: string, onChange: () => void) {
  ...
  const connect = async () => {
    try {
      const stored = sessionStorage.getItem(`purchase-ws:${purchaseId}`);
      sessionStorage.removeItem(`purchase-ws:${purchaseId}`);
      const token = stored ?? (await purchaseWsToken(purchaseId)).token;
      if (stopped) return;
