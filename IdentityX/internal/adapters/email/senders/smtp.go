...
type SMTPSender struct {
	logs   *zap.Logger
	tracer trace.Tracer
	queue  chan outbounds.Email
	cfg    SMTPConfig
}

var _ outbounds.Mailer = (*SMTPSender)(nil)

func NewSMTPSender(logs *zap.Logger, tracer trace.Tracer, cfg SMTPConfig) outbounds.Mailer {
	s := &SMTPSender{
		logs:   logs,
		tracer: tracer,
		queue:  make(chan outbounds.Email, 100),
		cfg:    cfg,
	}
	go s.worker()
	return s
...
func (s *SMTPSender) Send(ctx context.Context, email outbounds.Email) error {
	select {
	case s.queue <- email:
		return nil
...
func buildMIME(email outbounds.Email, from string) ([]byte, error) {
...
func (s *SMTPSender) sendSMTP(ctx context.Context, email outbounds.Email) error {
