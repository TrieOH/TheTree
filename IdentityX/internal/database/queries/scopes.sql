...
INSERT INTO scopes (type, parent_id, project_id, name, external_id)
VALUES ($1, $2, $3, $4, $5)
RETURNING *;
...
WITH RECURSIVE ancestors AS (
    ...
    SELECT scopes.id, scopes.parent_id, scopes.type, scopes.project_id, scopes.name, scopes.external_id, scopes.created_at, 0 as depth
    FROM scopes
    WHERE scopes.id = $1
    ...
    SELECT s.id, s.parent_id, s.type, s.project_id, s.name, s.external_id, s.created_at, a.depth + 1
    FROM scopes s
    INNER JOIN ancestors a ON s.id = a.parent_id
    WHERE a.parent_id IS NOT NULL
)
SELECT ancestors.id, ancestors.parent_id, ancestors.type, ancestors.project_id, ancestors.name, ancestors.external_id, ancestors.created_at
FROM ancestors
