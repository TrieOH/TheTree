...
type ProjectUserRepository interface {
	Register(ctx context.Context, user project_users.ProjectUser) (*project_users.ProjectUser, error)
	...
	Update(ctx context.Context, user project_users.ProjectUser, ownerID uuid.UUID) (*project_users.ProjectUser, error)
