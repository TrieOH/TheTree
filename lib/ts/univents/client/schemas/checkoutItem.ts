 ...
 */
import type { CheckoutAttendee } from './checkoutAttendee';
import type { PurchaseItemType } from './purchaseItemType';
import type { Uuid } from './uuid';

/**
 * One requested cart line. Ticket lines are one-per-person: `quantity` is fixed at 1 and `attendee` carries the assigned person (self or gifted). Product lines carry quantity and no attendee; program lines are quantity=1 (one session) with no attendee.
 */
export interface CheckoutItem {
  item_type: PurchaseItemType;
  item_id: Uuid;
  /**
     * Product lines may exceed 1; ticket and program lines must be 1.
     * @minimum 1
     */
  quantity: number;
  /** Required on ticket lines (the person the ticket is assigned to). */
