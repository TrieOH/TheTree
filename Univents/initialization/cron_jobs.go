...
func productHardDeleteJob(ctx context.Context, app *UniventsApp) {
	db := app.DB
	...
	_, err := app.Scheduler.NewJob(
		...
		gocron.NewTask(func(ctx context.Context, pool *pgxpool.Pool) {
			...
			products, err := queries.GetExpiredSoftDeletedProducts(ctx)
			if err != nil {
			...
			for _, product := range products {
				...
				for _, u := range urls {
					...
					if err := app.Minio.RemoveObject(ctx, bucket, key, minio.RemoveObjectOptions{}); err != nil {
			...
			if err := queries.MarkProductsHardDeleted(ctx, hardDeleted); err != nil {
