...
// seam: the SDK surfaces "no account for this email" / "unknown actor" as
// a 404 APIError, which is mapped to ErrActorNotFound so the checkout
// logic can branch on "no account yet" (email-only gift) vs "verification
// failed" (error).
...
// attendee resolution and gift claim. Lookups are scoped to the univents
// project (the client's project id).
func NewSDKActorResolver(actors *idx.ActorService) ActorResolver {
