...
// Operations set: bodies that decode go through the chain and die at the
// platform-only scope checker (no identity in context), while malformed
// bodies are rejected before the chain ever runs.
...
// body must now flow into the chain — here it is stopped at the
// platform-only scope checker (401) because the test carries no identity,
// which proves the decode passed. The schema is the exact draft-2020-12
// document a real client sent.
