...
- **Actors**: `GET /projects/{id}/actors`, `GET /projects/{id}/actors/{id}`, `GET /projects/{id}/actors/{email}:by_email` — project members or platform-level clients (the ops layer checks project membership; `createActor` stays platform-level only).
